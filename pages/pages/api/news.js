    ...
    <div>
      ...
      <div style={{minHeight:"100vh",background:"#0f172a",padding:12,fontFamily:"system-ui",color:"white"}}>
        ...
        <div style={{background:"#1e293b",borderRadius:12,padding:16,border:"1px solid #334155"}}>
          ...
          <div><div style={{fontSize:11,color:"#9ca3af",marginBottom:8}}>LIVE NEWS</div>{stockNews.length>0?stockNews.map(function(n,i){return(<div key={i} style={{background:i%2===0?"#111827":"transparent",borderRadius:6,padding:10,marginBottom:4}}><div style={{fontSize:12,fontWeight:600,marginBottom:4}}>{n.headline}</div><div style={{fontSize:10,color:"#94a3b8",marginBottom:4}}>{n.summary?n.summary.substring(0,150)+"...":""}</div><div style={{fontSize:9,color:"#6b7280"}}><a href={n.url} target="_blank" rel="noopener noreferrer" style={{color:"#22d3ee",textDecoration:"none"}}>{n.source}</a> | {timeAgo(n.time)}</div></div>);}):(<div style={{fontSize:11,color:"#64748b",padding:10}}>No recent news. Hit Refresh to load.</div>)}</div>
        </div>
